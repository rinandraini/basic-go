...
func main() {
	...
	switch {
	case sizeQueue < 6:
		for i := 0 + 1; i < sizeQueue+1; i++ {
			fmt.Printf("Antrian %d\t: membeli 1kg beras dengan kualitas [SUPER]\n", i)
		}
	case sizeQueue >= 6 && sizeQueue < 11:

		for i := 0 + 1; i <= 5; i++ {
			fmt.Printf("Antrian %d\t: membeli 1kg beras dengan kualitas [SUPER]\n", i)
		}

		for k := 6; k <= sizeQueue; k++ {
			fmt.Printf("Antrian %d\t: membeli 1kg beras dengan kualitas [MEDIUM]\n", k)
		}

	case sizeQueue >= 11 && sizeQueue < 16:

		for i := 1; i < sizeQueue-1; i++ {
			fmt.Printf("Antrian %d \t: membeli 1kg beras dengan kualitas [SUPER]\n", i)
		}
